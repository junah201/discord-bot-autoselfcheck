...
async def 도움말(ctx):
    ...
    embed.add_field(name="?정보등록", value="?정보등록 [이름] [생년월일] [지역] [학교이름] [학교타입] [비밀번호]```?정보등록 홍길동 721027 서울시 길동고 고등학교 1234``````?정보등록 홍길동 050201 충청남도 길동중 중학교 2580```정보등록은 개인DM으로 하는 것을 보안상 추천 드립니다.", inline=False)
...
async def 관리자정보확인(ctx):
    if str(ctx.author.id) == '523017072796499968':
        channel = bot.get_channel(int(log_json_backup_channel))
        await channel.send(f"[{datetime.datetime.now().strftime('%Y-%m-%d')}]\n{host_name}")
        file = discord.File("user_data.json")
        await channel.send(file=file)
        #file = discord.File("auto_self_check.py")
        #await channel.send(file=file)
...
async def 관리자전체공지(ctx,*,msg):
    if str(ctx.author.id) == '523017072796499968':
        ...
        user = await bot.fetch_user(ctx.author.id)
        await user.send("관리자 권한이 없어 해당 명령어를 사용할 수 없습니다.")
token = 'ODQ2NjUwNjE4NzAxMjgzMzU5.YKym1Q.AdgTSMXe4mtn41btGR5X9_INM-4'
