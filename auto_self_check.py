...
end_msg = "\n\n개발자 : white201#0201 | [개발자 서버](https://discord.gg/bhJEbEgHED) | [초대링크](https://discord.com/api/oauth2/authorize?client_id=846650618701283359&permissions=2184703040&scope=bot)"
...
async def 재시작(ctx):
    if str(ctx.author.id) == '523017072796499968':
        await ctx.bot.close()
        await ctx.login(token, bot=True)
    else:
        user = await bot.fetch_user(ctx.author.id)
        await user.send("관리자 권한이 없어 해당 명령어를 사용할 수 없습니다.")
'''
