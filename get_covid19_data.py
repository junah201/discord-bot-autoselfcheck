...
async def get_covid19_data():
    url = f"http://openapi.data.go.kr/openapi/service/rest/Covid19/getCovid19SidoInfStateJson?serviceKey={covid19_api_key}&pageNo=1&numOfRows=10&startCreateDt={datetime.datetime.now().strftime('%Y%m%d')}&endCreateDt={datetime.datetime.now().strftime('%Y%m%d')}"
    headers = {'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.130 Safari/537.36'}
    response = requests.get(url,headers=headers)
    covid19_data = json.loads(json.dumps(xmltodict.parse(response.text), indent=4))
    return_data = {}
